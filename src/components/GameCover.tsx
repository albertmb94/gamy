...
/**
 * Portada generada para juegos sin imagen. Estética tipo reproductor:
 * esquinas muy redondeadas (2xl), paleta neutra y textura sutil.
 * `compact` para miniaturas; `large` para cabeceras a pantalla completa.
 */
export function GameCover({
  ...
  if (compact) {
    return (
      <div className={cn('w-full h-full flex items-center justify-center bg-gradient-to-br from-zinc-100 to-zinc-200 rounded-xl', className)}>
  ...
  return (
    <div className={cn('relative w-full h-full overflow-hidden bg-gradient-to-br from-zinc-50 to-zinc-200 rounded-2xl', className)}>
      <div
      ...
      <span className={cn('absolute top-3 left-3', large ? 'text-4xl' : 'text-2xl')}>{emoji}</span>
